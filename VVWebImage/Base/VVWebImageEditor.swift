//
//  VVWebImageEditor.swift
//  VVWebImage
//

import UIKit

public typealias VVWebImageEditMethod = (UIImage) -> UIImage?

public let vv_shareColorSpace = CGColorSpaceCreateDeviceRGB()
public let vv_ScreenScale = UIScreen.main.scale

private var shareCIContext: CIContext?
public var vv_shareCIContext: CIContext {
    var localContext = shareCIContext
    if localContext == nil {
        if #available(iOS 9.0, *) {
            localContext = CIContext(options: [CIContextOption.workingColorSpace : vv_shareColorSpace])
        } else {
            // CIContext.init(options:) will crash in iOS 8. So use other init
            localContext = CIContext(eaglContext: EAGLContext(api: .openGLES2)!, options: [CIContextOption.workingColorSpace : vv_shareColorSpace])
        }
...
public func vv_clearCIContext() { shareCIContext = nil }

/// Creates a VVWebImageEditor to crop image
///
/// - Parameter rect: rect to crop, mesured in points
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorCrop(with rect: CGRect) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_croppedImage(with: rect) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.crop.rect=\(rect)", edit: edit)
}

/// Creates a VVWebImageEditor to resize image to the specific size
///
/// - Parameter size: size to resize
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorResize(with size: CGSize) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_resizedImage(with: size) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.resize.size=\(size)", edit: edit)
}

/// Creates a VVWebImageEditor to resize image with view size and content mode.
...
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorResize(with displaySize: CGSize, contentMode: UIView.ContentMode) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_resizedImage(with: displaySize, contentMode: contentMode) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.resize.size=\(displaySize),contentMode=\(contentMode.rawValue)", edit: edit)
}

/// Creates a VVWebImageEditor to resize image with view size and fill content mode.
/// Some portion of the image may be clipped.
/// Use the method to display the valid portion of image to save memory.
///
/// - Parameters:
...
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorResize(with displaySize: CGSize, fillContentMode: UIView.VVFillContentMode) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_resizedImage(with: displaySize, fillContentMode: fillContentMode) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.resize.size=\(displaySize),fillContentMode=\(fillContentMode)", edit: edit)
}

/// Creates a VVWebImageEditor to rotate image
///
/// - Parameters:
///   - angle: angle (degree) to rotate
///   - fitSize: true to change image size to fit rotated image, false to keep image size
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorRotate(withAngle angle: CGFloat, fitSize: Bool) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_rotatedImage(withAngle: angle, fitSize: fitSize) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.rotate.angle=\(angle),fitSize=\(fitSize)", edit: edit)
}

/// Creates a VVWebImageEditor to flip image horizontally and/or vertically
///
/// - Parameters:
///   - horizontal: whether to flip horizontally or not
///   - vertical: whether to flip vertically or not
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorFlip(withHorizontal horizontal: Bool, vertical: Bool) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_flippedImage(withHorizontal: horizontal, vertical: vertical) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.flip.horizontal=\(horizontal),vertical=\(vertical)", edit: edit)
}

/// Creates a VVWebImageEditor to tint image with color
///
/// - Parameters:
///   - color: color to draw
///   - blendMode: blend mode to use when compositing the image
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorTint(with color: UIColor, blendMode: CGBlendMode = .normal) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_tintedImage(with: color, blendMode: blendMode) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.tint.color=\(color),blendMode=\(blendMode.rawValue)", edit: edit)
}

/// Creates a VVWebImageEditor to tint image with gradient color
///
/// - Parameters:
...
///   - blendMode: blend mode to use when compositing the image
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorGradientlyTint(with colors: [UIColor],
                                         locations: [CGFloat],
                                         start: CGPoint = CGPoint(x: 0.5, y: 0),
                                         end: CGPoint = CGPoint(x: 0.5, y: 1),
                                         blendMode: CGBlendMode = .normal) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_gradientlyTintedImage(with: colors,
                                                             locations: locations,
                                                             start: start,
                                                             end: end,
                                                             blendMode: blendMode) {
    ...
    return VVWebImageEditor(key: "com.waqu.VVWebImage.gradientlyTint.colors=\(colors),locations=\(locations),start=\(start),end=\(end),blendMode=\(blendMode.rawValue)", edit: edit)
}
...
/// Creates a VVWebImageEditor to overlay image with another image
///
/// - Parameters:
///   - overlayImage: image at the top as an overlay
///   - blendMode: blend mode to use when compositing the image
///   - alpha: opacity of overlay image, specified as a value between 0 (totally transparent) and 1 (fully opaque)
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorOverlay(with overlayImage: UIImage, blendMode: CGBlendMode = .normal, alpha: CGFloat) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_overlaidImage(with: overlayImage, blendMode: blendMode, alpha: alpha) { return currentImage }
        return image
    }
    return VVWebImageEditor(key: "com.waqu.VVWebImage.overlay.image=\(overlayImage),blendMode=\(blendMode.rawValue),alpha=\(alpha)", edit: edit)
}

/// Creates a VVWebImageEditor for common use
///
/// - Parameters:
...
/// - Returns: a VVWebImageEditor variable
public func vv_imageEditorCommon(with displaySize: CGSize,
                                 fillContentMode: UIView.VVFillContentMode = .center,
                                 maxResolution: Int = 0,
                                 corner: UIRectCorner = UIRectCorner(rawValue: 0),
                                 cornerRadius: CGFloat = 0,
                                 borderWidth: CGFloat = 0,
                                 borderColor: UIColor? = nil,
                                 backgroundColor: UIColor? = nil) -> VVWebImageEditor {
    let edit: VVWebImageEditMethod = { (image) in
        if let currentImage = image.vv_commonEditedImage(with: displaySize,
    ...
    let key = "com.waqu.VVWebImage.common.displaySize=\(displaySize),fillContentMode=\(fillContentMode),maxResolution=\(maxResolution),corner=\(corner),cornerRadius=\(cornerRadius),borderWidth=\(borderWidth),borderColor=\(borderColor?.description ?? "nil"),backgroundColor=\(backgroundColor?.description ?? "nil")"
    return VVWebImageEditor(key: key, edit: edit)
}

public func vv_borderPath(with size: CGSize, corner: UIRectCorner, cornerRadius: CGFloat, borderWidth: CGFloat) -> UIBezierPath {
...
public func vv_drawForScaleDown(_ context: CGContext, sourceImage: CGImage) {
...
/// VVWebImageEditor defines how to edit and cache image in memory
public struct VVWebImageEditor {
    public var key: String
    public var edit: VVWebImageEditMethod
    
    /// Creates a VVWebImageEditor variable
    ///
    /// - Parameters:
    ///   - key: identification of editor
    ///   - needData: whether image data is necessary or not for editing
    ///   - edit: an edit image closure
    public init(key: String, edit: @escaping VVWebImageEditMethod) {
        self.key = key
        self.edit = edit
    }
}
...
public extension UIImage {
    ...
    func vv_croppedImage(with originalRect: CGRect) -> UIImage? {
        ...
        return _vv_croppedImage(with: rect)
    }
    ...
    func vv_resizedImage(with size: CGSize) -> UIImage? {
    ...
    ///   - contentMode: view content mode
    /// - Returns: a resized image
    func vv_resizedImage(with displaySize: CGSize, contentMode: UIView.ContentMode) -> UIImage? {
        if displaySize.width <= 0 || displaySize.height <= 0 { return nil }
        let rect = vv_rectToDisplay(with: displaySize, contentMode: contentMode)
        return _vv_croppedImage(with: rect)
    }
    ...
    ///   - fillContentMode: fill content mode specifying how content fills its view
    /// - Returns: a resized image
    func vv_resizedImage(with displaySize: CGSize, fillContentMode: UIView.VVFillContentMode) -> UIImage? {
        if displaySize.width <= 0 || displaySize.height <= 0 { return nil }
        let rect = vv_rectToDisplay(with: displaySize, fillContentMode: fillContentMode)
        return _vv_croppedImage(with: rect)
    }
    
    private func _vv_croppedImage(with rect: CGRect) -> UIImage? {
        ...
        if let ciimage = ciImage?.cropped(to: rect),
            let sourceImage = vv_shareCIContext.createCGImage(ciimage, from: ciimage.extent) {
            return UIImage(cgImage: sourceImage, scale: scale, orientation: imageOrientation)
        }
        return nil
    }
    ...
    func vv_rotatedImage(withAngle angle: CGFloat, fitSize: Bool) -> UIImage? {
    ...
    func vv_flippedImage(withHorizontal horizontal: Bool, vertical: Bool) -> UIImage? {
        UIGraphicsBeginImageContextWithOptions(size, false, scale)
        guard let context = UIGraphicsGetCurrentContext() else {
            UIGraphicsEndImageContext()
            return nil
    ...
    func vv_tintedImage(with color: UIColor, blendMode: CGBlendMode = .normal) -> UIImage? {
        UIGraphicsBeginImageContextWithOptions(size, false, scale)
        draw(at: .zero)
        color.setFill()
    ...
    func vv_gradientlyTintedImage(with colors: [UIColor],
                                         locations: [CGFloat],
                                         start: CGPoint = CGPoint(x: 0.5, y: 0),
                                         end: CGPoint = CGPoint(x: 0.5, y: 1),
        ...
        guard let context = UIGraphicsGetCurrentContext(),
            let gradient = CGGradient(colorsSpace: vv_shareColorSpace,
    ...
    ///   - alpha: opacity of overlay image, specified as a value between 0 (totally transparent) and 1 (fully opaque)
    /// - Returns: an overlaid image
    func vv_overlaidImage(with overlayImage: UIImage, blendMode: CGBlendMode = .normal, alpha: CGFloat) -> UIImage? {
        UIGraphicsBeginImageContextWithOptions(size, false, scale)
        draw(at: .zero)
    ...
    ///   - backgroundColor: background color of image
    /// - Returns: a VVWebImageEditor variable
    func vv_commonEditedImage(with displaySize: CGSize,
                                     fillContentMode: UIView.VVFillContentMode = .center,
                                     maxResolution: Int = 0,
                                     corner: UIRectCorner = UIRectCorner(rawValue: 0),
                                     cornerRadius: CGFloat = 0,
        ...
        return autoreleasepool { () -> UIImage? in
            guard displaySize.width > 0,
                ...
                let sourceImage = cgImage?.cropping(to: vv_rectToDisplay(with: displaySize, fillContentMode: fillContentMode)) else { return nil }
            ...
            } else if CGFloat(width) < displaySize.width * vv_ScreenScale {
                width = Int(displaySize.width * vv_ScreenScale)
                height = Int(displaySize.height * vv_ScreenScale)
            }
            ...
            if cornerRadius > 0 && corner.isSubset(of: .allCorners) && !corner.isEmpty {
                let clipPath = vv_borderPath(with: CGSize(width: width, height: height), corner: corner, cornerRadius: currentCornerRadius, borderWidth: currentBorderWidth)
            ...
            context.translateBy(x: 0, y: CGFloat(-height))
            if shouldScaleDown {
                vv_drawForScaleDown(context, sourceImage: sourceImage)
            } else {
            ...
            if let strokeColor = borderColor?.cgColor,
                borderWidth > 0 {
                let strokePath = vv_borderPath(with: CGSize(width: width, height: height), corner: corner, cornerRadius: currentCornerRadius, borderWidth: currentBorderWidth)
    ...
    func vv_rectToDisplay(with displaySize: CGSize, contentMode: UIView.ContentMode) -> CGRect {
        var rect = CGRect(origin: .zero, size: size)
        switch contentMode {
    ...
    ///   - fillContentMode: fill content mode specifying how content fills its view
    /// - Returns: image rect to display in image coordinate
    func vv_rectToDisplay(with displaySize: CGSize, fillContentMode: UIView.VVFillContentMode) -> CGRect {
        var rect = CGRect(origin: .zero, size: size)
        let sourceRatio = size.width / size.height
        let displayRatio = displaySize.width / displaySize.height
...
public extension UIView {
    /// VVFillContentMode specifies how content fills its view
    enum VVFillContentMode {
