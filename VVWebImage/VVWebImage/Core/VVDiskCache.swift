...
/// VVDiskCache is a key-value disk cache using least recently used algorithm
public class VVDiskCache {
    private let storage: VVDiskStorage
    private let sizeThreshold: Int
    private let queue: VVDispatchQueuePool
    private var costLimit: Int
    private var countLimit: Int
    private var ageLimit: TimeInterval
    
    /// Crreates a VVDiskCache object
    ///
    /// - Parameters:
    ...
    public init?(path: String, sizeThreshold threshold: Int) {
        if let currentStorage = VVDiskStorage(path: path) {
            storage = currentStorage
        } else {
            return nil
        }
        sizeThreshold = threshold
        queue = VVDispatchQueuePool.utility
        costLimit = .max
        countLimit = .max
    ...
    public func dataExists(forKey key: String, completion: @escaping VVImageCacheCheckDiskCompletion) {
        queue.async { [weak self] in
            guard let self = self else { return }
    ...
    public func store(_ data: Data, forKey key: String, completion: VVImageCacheStoreCompletion?) {
        queue.async { [weak self] in
            guard let self = self else { return }
    ...
    ///   - key: cache key
    ///   - completion: a closure called when storing is finished
    public func store(_ dataWork: @escaping () -> Data?, forKey key: String, completion: VVImageCacheStoreCompletion?) {
        queue.async { [weak self] in
            guard let self = self else { return }
    ...
    public func removeData(forKey key: String, completion: VVImageCacheRemoveCompletion?) {
        queue.async { [weak self] in
            guard let self = self else { return }
    ...
    public func clear(_ completion: VVImageCacheRemoveCompletion?) {
        queue.async { [weak self] in
            guard let self = self else { return }
            self.clear()
            completion?()
