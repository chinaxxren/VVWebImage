...
public class VVWebImageGIFCoder: VVAnimatedImageCoder {
    ...
    public func imageFrame(at index: Int, decompress: Bool) -> UIImage? {
        if let source = imageSource,
            ...
            if decompress {
                if let cgimage = VVWebImageImageIOCoder.decompressedImage(sourceImage) {
...
extension VVWebImageGIFCoder: VVImageCoder {
    public func canDecode(_ data: Data) -> Bool {
        return data.vv_imageFormat == .GIF
    }
    
    public func decodedImage(with data: Data) -> UIImage? {
        let image = VVAnimatedImage(vv_data: data, decoder: copy() as? VVAnimatedImageCoder)
    ...
    public func encodedData(with image: UIImage, format: VVImageFormat) -> Data? {
        ...
        if let animatedImage = image as? VVAnimatedImage,
            animatedImage.vv_imageFormat == .GIF {
            return animatedImage.vv_originalImageData
        }
    ...
    public func copy() -> VVImageCoder { return VVWebImageGIFCoder() }
}
