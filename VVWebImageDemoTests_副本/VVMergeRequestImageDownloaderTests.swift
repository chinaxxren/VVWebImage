//
//  VVMergeRequestImageDownloaderTests.swift
//  VVWebImageTests
//
...
class VVMergeRequestImageDownloaderTests: XCTestCase {
    var downloader: VVMergeRequestImageDownloader!
    var urls: [URL]!
    
    lazy var imageCoder: VVImageCoder = {
        return VVImageCoderManager()
    }()
    
    override func setUp() {
        downloader = VVMergeRequestImageDownloader(sessionConfiguration: .default)
    ...
    func testCancelTasks() {
        let expectation = self.expectation(description: "Wait for downloading image")
        var tasks: [VVImageDownloadTaskProtocol] = []
        for url in urls {
            let task = downloader.downloadImage(with: url, options: .none) { (data, error) in
                XCTFail()
            }
